...
func (r *Repository) AddEvent(ctx context.Context, day int, message string) error {
	_, err := r.db.Exec(ctx, `INSERT INTO events (day, message) VALUES ($1, $2)`, day, message)
	return err
}

func (r *Repository) ListEvents(ctx context.Context) ([]domain.Event, error) {
	rows, err := r.db.Query(ctx, `SELECT id, day, message FROM events ORDER BY id DESC LIMIT 50`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
